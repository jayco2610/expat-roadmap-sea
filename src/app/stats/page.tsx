...
const ACTIONS: [string, string][] = [
  ...
  ["guide_open", "Открытий гайдов"],
  ["cheatsheet", "Переходов к чеклисту"],
  ["ai_question", "Вопросов ИИ"],
  ["pro_click", "Интерес к Pro"],
...
export default async function Stats({ searchParams }: { searchParams: Promise<{ key?: string }> }) {
  ...
  const actions = await Promise.all(ACTIONS.map(async ([k, label]) => ({ label, ...(await row("ex", k)) })));
  const pages = await Promise.all(PAGES.map(async ([k, label]) => ({ label, ...(await row("ex:path", k)) })));
  ...
  return (
    <main style={{ padding: "48px 24px", maxWidth: 760, margin: "0 auto" }}>
      <h1 style={{ fontSize: 32, margin: 0 }}>Счётчик Expat Roadmap</h1>
