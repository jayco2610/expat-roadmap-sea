...
export default async function EventDetailPage({ params }: Props) {
  ...
  return (
    <PageShell>
      <Link href="/events" className="mb-6 inline-flex text-sm text-[#0071e3] hover:underline">
        ← All events
      </Link>
      ...
      <article className="card-apple max-w-2xl p-6 sm:p-8">
        ...
        <p className="mt-4 text-sm text-[#6e6e73] dark:text-[#a1a1a6]">
          Hosted by{" "}
          <Link href={`/community/${event.author.id}`} className="text-[#0071e3] hover:underline">
        ...
        <section className="mt-8 border-t border-black/8 pt-6 dark:border-white/10">
          ...
          {going.length > 0 ? (
            <ul className="mt-4 space-y-1 text-sm text-[#6e6e73] dark:text-[#a1a1a6]">
              {going.map((r) => (
                <li key={r.id}>
                  <Link href={`/community/${r.profile.id}`} className="text-[#0071e3] hover:underline">
